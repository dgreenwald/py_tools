...
class TestTwoAxis:
    ...
    def test_single_legend(self, tmp_path):
        ...
        pc.two_axis(
            df, "a", "b", single_legend=True, loc_single="upper left", filepath=fp
        )
        assert os.path.exists(fp)
...
class TestDoubleHist:
    ...
    def test_topcode_bottomcode(self, tmp_path):
        ...
        bins = np.linspace(-3, 3, 8)
        result = pc.double_hist(
            df1, df2, var="x", bins=bins, topcode=True, bottomcode=True, filepath=fp
        )
        assert result is True
...
class TestMultiHist:
    ...
    def test_use_bar(self, tmp_path):
        ...
        result = pc.multi_hist(
            dfs, labels=["A", "B"], xvar="x", bins=bins, use_bar=True, filepath=fp
        )
        assert result is True
    ...
    def test_density_false(self, tmp_path):
        ...
        result = pc.multi_hist(
            dfs, labels=["A", "B"], xvar="x", density=False, filepath=fp
        )
        assert result is True
...
class TestScatter:
    ...
    def test_include45(self, tmp_path):
        ...
        pc.scatter(
            df, "y", "x", labels={"x": "X", "y": "Y"}, include45=True, filepath=fp
        )
        assert os.path.exists(fp)
