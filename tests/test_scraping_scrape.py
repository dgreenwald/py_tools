...
class TestGetSoup:
    def _mock_response(self, status_code=200, content=b"<html><body><p>Hi</p></body></html>"):
        resp = MagicMock()
    ...
    def test_parses_html_structure(self):
        html = b"<html><head><title>T</title></head><body><h1>Heading</h1></body></html>"
        with patch("requests.get", return_value=self._mock_response(200, html)), \
             patch("time.sleep"):
...
class TestUrlToNltk:
    ...
    def _mock_response(self, status_code=200, content=b"<html><body><p>The quick brown fox</p></body></html>"):
        resp = MagicMock()
        resp.status_code = status_code
        resp.content = content
        return resp
