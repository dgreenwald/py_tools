...
def load_county_year(year, data_dir=default_dir, reimport=False):
    ...
    if reimport or not os.path.exists(pkl_file):
        ...
        if year <= 2010:
            df_t = import_county_year_to_2010(year, data_dir=default_dir)
        else:
...
def import_county_year_from_2011(year, data_dir=default_dir):
    ...
    df_by_cat['county_code'] = pd.to_numeric(df_by_cat['county_code'], errors='coerce').astype(np.int64)
    df_by_cat = df_by_cat.dropna(subset=['county_code'])

    df_t = df_by_cat.groupby(['county_code']).sum().drop(['agi_stub'], axis=1).reset_index()

    df_t['date'] = '{0:d}-01-01'.format(year)

    return df_t

def import_county_year_to_2010(year, data_dir=default_dir):

    year_dir = data_dir + str(year) + 'CountyIncome/'
    
    if year <= 2009:

        names = ['DROP', 'state_code', 'county_code', 'county', 'n_returns', 'n_exemptions', 'agi', 'wagesal', 'dividends', 'interest']
        drop_list = ['DROP']
        ...
        names = [
            'county_code', 'categories', 'n_returns', 'n_joint', 'n_prepared',
...
#            pd.read_excel(filename, skiprows=9, header=None, names=names)
    ...
    df_t['county_code'] = pd.to_numeric(df_t['county_code'], errors='coerce')
    df_t = df_t.drop(drop_list, axis=1).dropna(subset=['county_code'])
    for name in ['county_code']:
...
def load_county(data_dir=default_dir, reimport=False, reimport_year=False):
    ...
    if reimport or not os.path.exists(pkl_file):
        ...
        df = df.set_index(['county_code', 'date']).sort_index()
