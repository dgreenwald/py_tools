...
def collapse_quantile(df, by_list, weight_var=None, var_list=None, q=0.5, **kwargs):
    ...
    **kwargs
    ...
    df_out = df.groupby(by_list).apply(
        get_weighted_quantile_inner, var_list, weight_var, q, **kwargs
    )
...
def concat(collapser_list, check=True):
    ...
    col = Collapser(dfc=dfc, var_list=var_list, weight_var=weight_var, by_list=by_list)
...
def collapse_multiquantile(
    df, by_list, q_list, weight_var=None, var_list=None, **kwargs
):
    ...
    **kwargs
    ...
    dfout_list = []
    for q in q_list:
        dfout = collapse_quantile(
            df, by_list, weight_var=weight_var, var_list=var_list, q=q, **kwargs
        )
    ...
    return pd.concat(
        [
            collapse_multiquantile(
                df, by_list, q_list=q_list, weight_var=weight_var, var_list=[var]
            )
            for var, weight_var in weight_dict.items()
        ],
        axis=1,
...
class Collapser:
    ...
    **kwargs
    ...
    def __init__(
        self, df=None, var_list=None, weight_var=None, by_list=None, dfc=None, **kwargs
    ):
        ...
        **kwargs
        ...
        if df is not None:
            self.set_data(
                df, var_list, weight_var, by_list=by_list, inplace=True, **kwargs
            )
        else:
            self.dfc = dfc
    ...
    def set_data(
        ...
        df_denom = df_denom.rename(
            {var: var + "_denom" for var in df_denom.columns}, axis=1
        )
    ...
    def get_data(self, weight_suffix=False, include_denom=False, as_sum=False):
        ...
        if include_denom:
            df_out = pd.concat(
                [df_out, self.dfc[[var + "_denom" for var in self.var_list]]], axis=1
            )
    ...
    def get_numerators_and_denominators(self):
        ...
        """
        df_num = self.dfc[[var + "_num" for var in self.var_list]].rename(
            {var + "_num": var for var in self.var_list}, axis=1
        )
        df_denom = self.dfc[[var + "_denom" for var in self.var_list]].rename(
            {var + "_denom": var for var in self.var_list}, axis=1
        )
